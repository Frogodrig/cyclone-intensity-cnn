...
# This file downloads the satellite images
#
# A general outline of this file:
# - Loops through each year and downloads .tar.gz files containing satellite images for Atlantic and Pacific cyclones
# - Only extracts files from the .tar.gz files that contain images of hurricanes that we know the wind speed of
# - When the script is finished, the "Satellite Imagery" folder contains all netcdf files, which will hold the satellite images
...
def download_HurSat(years):
    ...
    # cyclone in the Atlantic and Central/Eastern Pacific 
    ...
    for year in years:
        ...
        for link in year_directory_soup.find_all('a'):
            if link.get('href').endswith('tar.gz'):
                year_directory_file_urls.append(year_directory_url + '/' + link.get('href'))
        print('\n' + year + ' file loaded.')

        # This variable will be used later in displaying the progress bar
        files_processed = 0
        for storm_file_url in year_directory_file_urls:
            # Determine whether the best track dataset has information about this particular cyclone. This filters
            # out cyclones not present in the Atlantic or Pacific, since the best track dataset doesn't have
            # information for those cyclones.
            ...
            if file_has_match_in_best_track:
                ...
                # satellite image of a cyclone at a moment in time
                ...
                for file_name in tar.getnames():
                    ...
                    # Determine whether another image of this cyclone at this exact time has already been extracted
                    # from the .tar.gz
                    is_redundant = '.'.join(file_name.split('.')[:6]) in file_prefixes_in_directory

                    # If the requirements are met, extract the netcdf file from this .tar.gz and save it
...
if __name__ == "__main__":
    ...
    YEARS_TO_DOWNLOAD = ['2007','2008','2009','2010','2011','2012', '2013', '2014', '2015', '2016']
